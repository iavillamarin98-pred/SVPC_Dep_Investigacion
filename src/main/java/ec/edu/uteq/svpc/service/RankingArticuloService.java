...
public class RankingArticuloService {
    ...
    public List<RankingArticuloDTO> obtenerRankingArticulos(Long idProceso) {
        ...
        String sql = """
                SELECT
                    ROW_NUMBER() OVER (ORDER BY COALESCE(v.puntaje_articulos, 0) DESC) AS puesto,
                    d.cedula,
                    d.nombres,
                    d.apellidos,
                    d.carrera,
                ...
                FROM valoraciones v
                JOIN docentes d ON d.id_docente = v.id_docente
                WHERE v.id_proceso = ?
        ...
        return jdbcTemplate.query(sql, (rs, rowNum) -> new RankingArticuloDTO(
                ...
                rs.getBigDecimal("puntaje_articulos")), idProceso);
    }
}